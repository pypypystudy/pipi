...
unsigned char msg[282] = { }; 
*/
    cout<<"send request packet1"<<endl;
...
0x6F, 0x6E, 0x74, 0x65, 0x6E, 0x74, 0x2D, 0x4C, 0x65, 0x6E, 0x67, 0x74, 0x68, 0x3A, 0x20};
    ...
    unsigned char http_head2[28] = {
0x0D, 0x0A, 0x43, 0x6F, 0x6E, 0x6E, 0x65, 0x63, 0x74, 0x69, 0x6F, 0x6E, 0x3A, 0x20,
...
0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    ...
    int http_content_length = decode_length + 12;
    char content_length_buf[8] = {0};
    int actual_length = 0;
    int2ascii(content_length_buf, http_content_length, &actual_length);

    //get length
    *length = decode_length + 95 + 28 + 8 + 4 + actual_length;

    unsigned char *buffer_ptr = buffer;
    //copy http head 1
    memcpy(buffer_ptr, http_head1, 95);
    buffer_ptr += 95;

    //copy http_content length = decode_length + 12 in ASCII
    memcpy(buffer_ptr, content_length_buf, actual_length);
    buffer_ptr += actual_length;

    //copy http head 2
    memcpy(buffer_ptr, http_head2, 28);
    buffer_ptr += 28;
...
void PP_Downloader::int2ascii(char *buf, int len_in, int *len_out)
{
    int count = 0;
    int length = len_in;
    while (0 != length)
